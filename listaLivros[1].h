...
void CriarVetorLivros(PNodo T, Livro *vetor, int *tam){
  while(T!=NULL){
    *tam = *tam + 1;
    vetor[*tam-1]=T->Elemento;
    T = T->Prox;
  }
}
...
void ElementosLivroqtd (PNodo T, int N, int *qtd){
  *(qtd)=-1;
  while(T!=NULL){
    if(T->Elemento.ISBN == N){
    *(qtd) = T->Elemento.Quantidade;
    }
    T = T->Prox;
  }
}