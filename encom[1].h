...
void guardar_enc(PNodoFila A){
  ...
  while(A != NULL){
    fprintf(ft,"%i %i %i %i %i %i %i %f \n",A->Elemento.numeroencomenda,A->Elemento.numerolivro,A->Elemento.numerocliente,A->Elemento.dia,A->Elemento.mes,A->Elemento.ano,A->Elemento.num,A->Elemento.preco);
    A = A->Prox;
  }
}
...
PNodoFila abrirficheiroencomendas(int *p){
  ...
  while(fscanf(ft,"%i %i %i %i %i %i %i %f",&k.numeroencomenda ,&k.numerolivro,&k.numerocliente,&k.dia,&k.mes,&k.ano,&k.num,&k.preco)==8){ 
    *p = k.numeroencomenda;
    q = Juntar(k,q);
  }
  return q;
